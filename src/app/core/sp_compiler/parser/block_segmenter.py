...
Usage:
    from app.core.sp_compiler.parser import tokenize, segment_blocks
...
class _TS:
    ...
    def collect_until_semicolon(self) -> list[Token]:
        ...
        while not self.eof:
            ...
            if t.type == TokenType.PUNCTUATION and t.value == ";":
                self.advance()
                break
            if t.type == TokenType.KEYWORD and _is_block_boundary(t.value.upper()):
                break
            collected.append(t)
            self.advance()
        return collected
    ...
    def _pass1(self) -> list[SemanticBlock]:
        ...
        while not self.ts.eof:
            ...
            if kw == "BEGIN":
                ...
                blocks.append(SemanticBlock(
                    BlockType.SQL, tokens=tuple(self.ts.collect_until_semicolon()),
                    source_line=token.line))
